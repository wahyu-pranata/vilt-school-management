        ...
        <div class="mt-4">
            <div class="overflow-x-auto relative px-8">
                ...
                <form action="#" method="post" class="mt-4 w-1/2" @submit.prevent="submit">
                    <InputForm id="name" type="text" placeholder="Insert teacher name..." label-text="Teacher Name" v-model="(form.teacher_name)" />
                    <div class="mb-3">
                        <label for="group">Select Subject</label>
                        <select name="group_id" id="group" v-model="form.subject_id">
                            <option v-for="subject in subjects" :value="subject.id" :key="subject.id">{{ subject.subject_name }}</option>
                        </select>
                    </div>
...
let form = useForm({
    teacher_name: props.student ? props.student.student_name : null,
    subject_id: props.student ? props.student.group_id : null
})

const submit = () => {
    props.teacher ? form.put(`/dashboard/teacher/${props.teacher.id}`) : form.post(`/dashboard/teacher`)
}
</script>
